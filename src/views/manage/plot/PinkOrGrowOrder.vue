	...
	<div id="sys_params">
		...
		<main-content>
			<el-form :inline="true" :model="params" class="demo-form-inline">
				...
				<el-form-item label="">
					<el-select v-model="params.type" placeholder="根据订单类型筛选" clearable @change="search()">
			...
			<el-table :data="list" stripe style="width: 100%" v-loading="load">
				...
				<el-table-column prop="totalPrice" label="总价" align="center">
					<template slot-scope="scope">
						<span style="color: red;" v-if="scope.row.totalPrice==null">暂未生成</span>
						<span v-else>{{scope.row.totalPrice}}</span>
					</template>
				</el-table-column>
				<el-table-column prop="realPrice" label="实际价格" show-overflow-tooltip align="center">
					<template slot-scope="scope">
						<span style="color: red;" v-if="scope.row.realPrice==null">暂未生成</span>
				...
				<el-table-column label="操作" align="center" width="300">
					<template slot-scope="scope">
						<el-button type="primary" size="mini"
							@click="$router.push('/plot/pinkOrGrowOrder/detali?id='+scope.row.id)">
							<i class="iconfont icondaohangdizhi"></i>查看订单详情
						</el-button>

            <el-button type="success" size="mini" v-if="scope.row.hasTake === 1 && scope.row.type === 1 && scope.row.status === 1"
                       @click="noticeUserPickUp(scope.row.orderNum)">
              <i class="iconfont icondaohangdizhi"></i>通知用户自提
            </el-button>

<!-- 						<el-button type="info" size="mini" v-if="scope.row.hasTake === 1 && scope.row.type === 1"
							@click="noticeUserPickUp(scope.row.orderNum)">
							<i class="iconfont icondaohangdizhi"></i>确认用户已摘
						</el-button> -->
					</template>
				</el-table-column>
		...
		methods: {
			...
			getStatus(status) {
				switch (status) {
					...
					case 1:
						return "<font style='color: #167cf3'>待收货</font>"
					case 2:
						return "<font style='color: #e1b919'>已核验</font>"
					case 3:
						return "<font style='color: #67c23a'>待评价</font>"
					case 4:
						return "<font style='color: #67c23a'>已完成</font>"
					case -1:
						return "<font style='color: red'>已取消</font>"
					case -2:
						return "<font style='color: #e67e22'>已退款</font>"
				}
			},
			search() {
