...
<template>
	<div class="middle-container" v-loading="loading">
		<jat-fillter :option="filterOptions" v-model="filterData" @searchFilter="searchFilter"
			...
			<span slot="parentId">
				<el-select v-model="filterData.parentId" filterable remote size="small" clearable placeholder="添加人"
					:remote-method="remoteMethod" :loading="remoteLoading">
					<el-option v-for="item in supervisorAdOptions" :key="item.id" :label="item.name" :value="item.id">
					</el-option>
				</el-select>
			</span>
		</jat-fillter>
		<BasicTable :columns="table.columns" :headerOperates="headerOperates" :operates="operates" :data="table.data"
			operateWidth="120px" :pageSize="table.pageSize" :currentPage="table.currentPage" :total="table.total"
			...
			getList() {
				...
				request.post({
					...
					params: {
						...
						endTime: registerTime && registerTime.length > 0 ?
