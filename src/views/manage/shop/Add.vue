<template>
	<div>
		<main-head><i id="head" class="iconfont icondingdandingdanmingxishouzhimingxi"></i>商品添加
		...
		<el-card style="margin-top: 20px;margin-bottom: 100px;" shadow="hover">
			<el-form :model="addForm" :rules="rule" :label-width="labelWidth" size="mini" ref="add">
				...
				<el-form-item label="产品名称" prop="title">
					<el-input v-model="addForm.title" :maxlength="38" clearable placeholder="请输入产品名称,限字数38"></el-input>
				</el-form-item>
				...
				<el-form-item label="简介" prop="overview">
					<el-input v-model="addForm.overview" :maxlength="100" clearable type="textarea" placeholder="请输入简介,限字数100"></el-input>
				</el-form-item>
				...
				<el-switch style="margin: 30px 0;" v-model="addForm.hasSpec" @change="specChange" active-text="有规格"
					inactive-text="无规格" :inactive-value="false" :active-value="true">
				</el-switch>

				<!-- 规格设置 -->
				<el-row class="row-bg" type="flex" justify='space-around' v-show="addForm.hasSpec" style="">
					<el-col>
						<!-- 规格分类 -->
				...
				<!-- 规格参数信息 -->
				<el-row class="row-bg" type="flex" justify='space-around' style="" v-if="addForm.hasSpec">
					<span style="margin-bottom: 10px;">规格参数信息:</span>
					<!-- 规格信息  -->
					<el-table :data="tableDataAlone" style="width: 100%" border>
						<el-table-column prop="describe" label="规格描述" min-width="230" v-if="addForm.hasSpec">
						</el-table-column>

						<el-table-column prop="originPrice" label="原价" min-width="230" v-if="addForm.hasSpec">
						...
						<el-table-column prop="score" min-width="230" label="库存" v-if="addForm.hasSpec">
				...
				this.$nextTick(() => {
					...
					this.$refs["add"].validate((valid) => {
						if (valid) {
							...
							this.addForm.hasSpec = this.addForm.hasSpec ? 1 : -1
