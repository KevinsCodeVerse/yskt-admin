		...
		methods: {
          ...
          'image', // 插入图片
				...
				this.editor.config.customUploadImg = (resultFiles, insertImgFn) => {
					...
					//判断上传图片请求是否携带id
					let flag = this.uploadImgServer.indexOf("?") == -1;
					this.$axios.post(this.uploadImgServer, formData, {
							"Content-Type": "multipart/form-data"
						})
						.then(response => {
							if (response.data.message == "success") {
								let data = response.data.result.uploads;
								let id = response.data.result.id;
								for (let i = 0; i < data.length; i++) {
									//图片回调							
									insertImgFn(data[i])
									this.$emit("uploadSuccess", data[i])
									if (flag) {
										console.log("id不存在")
										this.$emit("idUrl", id)
									}
