		...
		<div class="flex-box header_top">
			<div style="display: flex; align-items: center;" :style="getWidth()">
				<div class="flexible_model">
					<i class="el-icon-s-fold flexible" v-show="$store.state.navOpen"
						@click="$store.state.navOpen = !$store.state.navOpen"></i>
					<i class="el-icon-s-unfold flexible" v-show="!$store.state.navOpen"
						@click="$store.state.navOpen = !$store.state.navOpen"></i>
				</div>

				<el-breadcrumb>
					<el-breadcrumb-item v-for="(data, index) in breadcrumbs" :key="index" :to="data.path">{{data.name}}
					</el-breadcrumb-item>
				</el-breadcrumb>
			</div>
